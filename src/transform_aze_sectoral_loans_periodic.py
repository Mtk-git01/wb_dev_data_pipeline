...
from src.config import AZE_SECTORAL_LOANS_SOURCE_NAME

def transform_aze_sectoral_loans_periodic(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    required = ['country_iso', 'period_date', 'period_type', 'real_sector_loans_total_mn_azn']
    ...
    df["source_name"] = AZE_SECTORAL_LOANS_SOURCE_NAME
