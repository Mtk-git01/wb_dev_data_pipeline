...
from src.config import AZE_MOVABLE_PROPERTY_REGISTRY_SOURCE_NAME

def transform_aze_movable_property_registry_periodic(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    required = ['country_iso', 'period_date', 'period_type']
    ...
    df["source_name"] = AZE_MOVABLE_PROPERTY_REGISTRY_SOURCE_NAME
