...
from src.config import AZE_BUSINESS_PORTFOLIO_SOURCE_NAME

def transform_aze_business_portfolio_periodic(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    required = ['country_iso', 'period_date', 'period_type', 'business_loans_total_mn_azn']
    ...
    df["source_name"] = AZE_BUSINESS_PORTFOLIO_SOURCE_NAME
