...
from src.config import AZE_PAYMENT_SERVICE_MONTHLY_SOURCE_NAME

def transform_aze_payment_service_monthly(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    required = ['country_iso', 'month', 'atm_total_units', 'pos_total_units']
    ...
    df["source_name"] = AZE_PAYMENT_SERVICE_MONTHLY_SOURCE_NAME
