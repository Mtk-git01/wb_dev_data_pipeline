...
from src.config import AZE_NPL_STRUCTURE_SOURCE_NAME

def transform_aze_npl_structure_periodic(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    required = ['country_iso', 'period_date', 'period_type', 'npl_total_mn_azn']
    ...
    df["source_name"] = AZE_NPL_STRUCTURE_SOURCE_NAME
