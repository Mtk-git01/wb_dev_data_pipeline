...
from src.config import AZE_CARD_TRANSACTIONS_MONTHLY_SOURCE_NAME

def transform_aze_card_transactions_monthly(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    required = ['country_iso', 'month', 'payment_cards_total_thousand', 'card_txn_count_thousand']
    ...
    df["source_name"] = AZE_CARD_TRANSACTIONS_MONTHLY_SOURCE_NAME
