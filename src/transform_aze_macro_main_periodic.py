...
from src.config import AZE_MACRO_MAIN_PERIODIC_SOURCE_NAME

def transform_aze_macro_main_periodic(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    required = ['country_iso', 'period_date', 'period_type', 'nominal_gdp_mn_azn']
    ...
    df["source_name"] = AZE_MACRO_MAIN_PERIODIC_SOURCE_NAME
