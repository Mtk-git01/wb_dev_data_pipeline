...
from src.config import AZE_NATIONAL_PAYMENT_SYSTEMS_SOURCE_NAME

def transform_aze_national_payment_systems_periodic(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    required = ['country_iso', 'period_date', 'period_type', 'rtgs_txn_count_thousand']
    ...
    df["source_name"] = AZE_NATIONAL_PAYMENT_SYSTEMS_SOURCE_NAME
