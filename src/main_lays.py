...
from src.config import (
    PROJECT_ID,
    DATASET_ID,
    LAYS_TABLE_COUNTRY_YEAR,
    LAYS_TABLE_COUNTRY_LATEST,
)
from src.extract_lays import (
    get_world_countries_only,
    fetch_indicator_for_countries,
)
from src.transform_lays import (
...
def main() -> None:
    ...
    country_year_df.to_csv(
        output_dir / "lays_country_year.csv",
        index=False
    )
    country_latest_df.to_csv(
        output_dir / "lays_country_latest.csv",
        index=False
    )

    errors_year, warnings_year = validate_country_year_table(country_year_df)
    print_validation_results(errors_year, warnings_year, "lays_country_year")

    errors_latest, warnings_latest = validate_country_year_table(country_latest_df)
    print_validation_results(errors_latest, warnings_latest, "lays_country_latest")

    if errors_year or errors_latest:
        raise ValueError("Validation failed for LAYS tables")

    upload_to_bigquery(
        country_year_df,
        project_id=PROJECT_ID,
        dataset_id=DATASET_ID,
        table_id=LAYS_TABLE_COUNTRY_YEAR,
    )
    ...
    upload_to_bigquery(
        ...
        project_id=PROJECT_ID,
        dataset_id=DATASET_ID,
        table_id=LAYS_TABLE_COUNTRY_LATEST,
    )
